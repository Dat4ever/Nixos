...
for _oc_pid in $(pgrep -f 'opencode' 2>/dev/null || true); do
  ...
  _oc_args=()
  mapfile -t -d '' _oc_args < "/proc/$_oc_pid/cmdline" 2>/dev/null || true
  _oc_argv0="${_oc_args[0]:-}"
  if [[ ${#_oc_args[@]} -eq 1 && ( "$_oc_argv0" == "opencode" || "$_oc_argv0" == */opencode ) ]]; then
    kill -USR2 "$_oc_pid" 2>/dev/null || true
  fi
done
...
if command -v dconf >/dev/null 2>&1; then
  ...
  dconf write /org/gnome/desktop/interface/cursor-size 32 2>/dev/null || true
